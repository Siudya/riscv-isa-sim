...
static void sdcard_io_handler(uint32_t offset, int len, bool is_write) {
  ...
  switch (idx) {
    ...
    case SDDATA:
       // TODO
       if (!write_cmd) {
          if(fread(&base[SDDATA], 4, 1, fp) != 4)
            printf("SDCARD: Failed to read enough data!!");
       }
       addr += 4;
       break;
...
void sdcard_t::init() {
  ...
  // TODO: to be set to an actual image
  fp = fopen("/dev/zero","r");
  //assert(fp);
}
...
std::string sdcard_generate_dts(const sim_t *sim) {
  std::stringstream s;
  s << "  sdhci: mmc {\n"
...
sdcard_t *sdcard_parse_from_fdt(const void *fdt, const sim_t *sim,
                                          reg_t *base, const std::vector<std::string>& sargs) {
  *base = SDCARD_BASE;
  return new sdcard_t();
}

REGISTER_DEVICE(sdcard, sdcard_parse_from_fdt, sdcard_generate_dts)
