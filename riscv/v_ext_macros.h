...
#define VI_LOOP_ELEMENT_SKIP_NO_VMA_CHECK(BODY) \
  VI_MASK_VARS \
  bool skip = false; \
  if (insn.v_vm() == 0) { \
    BODY; \
...
//
#define VI_GENERAL_LOOP_CARRY_BASE \
  require(P.VU.vsew >= e8 && P.VU.vsew <= e64); \
  require_vector(true); \
  reg_t vl = P.VU.vl->read(); \
  reg_t UNUSED sew = P.VU.vsew; \
...
#define VI_VFP_NCVT_FP_TO_FP(BODY32, BODY64, \
  ...
  switch (P.VU.vsew) { \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_FP_PARAMS(32, 16), CHECK32, BODY32, 2); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_FP_PARAMS(64, 32), CHECK64, BODY64, 2); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_NCVT_INT_TO_FP(BODY32, BODY64, \
  ...
  switch (P.VU.vsew) { \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(32, 16, sign), CHECK32, BODY32, 2); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(64, 32, sign), CHECK64, BODY64, 2); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_NCVT_FP_TO_INT(BODY16, BODY32, BODY64, \
  ...
  switch (P.VU.vsew) { \
    case e8: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(16, 8, sign), CHECK16, BODY16, 2); } \
      break; \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(32, 16, sign), CHECK32, BODY32, 2); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(64, 32, sign), CHECK64, BODY64, 2); } \
      break; \
    default: \
      require(0); \
      break; \
